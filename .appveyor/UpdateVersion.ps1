...
Function SetVersion() {
    Write-Host "Set commit version: $env:APPVEYOR_BUILD_VERSION" -ForegroundColor "Yellow"
    RegexReplaceFile -file "AutoSaliens\UpdateChecker.cs" -id "VERSION" -replace $env:APPVEYOR_BUILD_VERSION
}

Function SetBranch() {
    Write-Host "Set commit version: $env:APPVEYOR_REPO_BRANCH" -ForegroundColor "Yellow"
    RegexReplaceFile -file "AutoSaliens\UpdateChecker.cs" -id "BRANCH" -replace $env:APPVEYOR_BUILD_VERSION
}

Function SetDate() {
    Write-Host "Set commit date: $env:APPVEYOR_REPO_COMMIT_TIMESTAMP" -ForegroundColor "Yellow"
